...
class DQNAgent(Agent):
    ...
    def __init__(self, env: "TradingEnv", policy_network_type="MLP", policy_network: tf.keras.Model = None):
        self.env = env
    ...
    def _build_policy_network(self):
        ...
        if self.policy_network_type == "LSTM":
            sequence_len, n_features = self.observation_shape
        ...
        elif self.policy_network_type == "MLP":
            network = tf.keras.Sequential(
                [
                    tf.keras.layers.InputLayer(input_shape=self.observation_shape),
                    tf.keras.layers.Conv1D(
            ...
            raise ValueError(f"Unknown policy network type: '{self.policy_network_type}'")
    ...
    def save(self, path: str, **kwargs):
        ...
        path = path / f"model_{str(episode).zfill(2)}.hdf5"
