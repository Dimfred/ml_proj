...
class Plotter(BaseRenderer):
    ...
    def _create_trade_annotations(
        ...
        """
        trades = {k: trade for k, trade in trades.items() if trade[0].step > self.min_step}
    ...
    def render_env(
        ...
        _performance = performance.iloc[self.min_step:]
        if not self.fig:
            self._create_figure(_performance.keys())
        ...
        _price_history = price_history.iloc[self.min_step:]
        # print(_price_history)
        self._price_chart.update(
            dict(
                open=_price_history["open"],
                high=_price_history["high"],
                low=_price_history["low"],
        ...
        self._volume_chart.update({"y": _price_history["volume"]})

        for trace in self.fig.select_traces(row=3):
            trace.update({"y": _performance[trace.name]})

        _net_worth = net_worth.iloc[self.min_step:]
        self._net_worth_chart.update({"y": _net_worth})
